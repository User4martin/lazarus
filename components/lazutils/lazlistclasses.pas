...
type
  ...
  *)
  ...
  TLazListAspectCapacityExp0x8000 = object(TLazListAspectCapacityFixed)
  public
  ...
  generic TGenListAspectCapacityCallback<TFALLBACK> = object(TLazListAspectCapacityFixed)
  public
  ...
  TLazListAspectCapacityCallback = specialize TGenListAspectCapacityCallback<TLazListAspectCapacityFixed>;
...
//    TPageSize = specialize TLazListAspectItemSiz<TPageType>;
  ...
  TLazPagedListObjParent = specialize TLazPagedListObjBase<Pointer, TLazListAspectVarItemSize> deprecated;
...
begin
  ...
  Result := TPItemT(@MPtr^.Data + (NewIdx * FItemSize.ItemSize));

  NewMem.Count := NewCnt;
  FCapacityAccessor.WriteCapacity(MPtr, AValue);
  assert((NewIdx >= 0) and (NewIdx <= AValue - NewCnt), 'TGenLazShiftList._SetCapacityEx: (NewIdx >= 0) and (NewIdx <= AValue - NewCnt)');
  ...
  if Cnt <> 0 then begin
    ...
    if AnInsertSize < 0 then begin
      if AnInsertPos < NewCnt then begin
        ...
        Move(PSource^, Result^, (Cardinal(NewCnt - AnInsertPos) * FItemSize.ItemSize));
      end;
    end
    else
      if AnInsertPos < Cnt then begin
        Move(PSource^,
             //(PByte(Result) + (Cardinal(AnInsertPos + AnInsertSize) * FItemSize.ItemSize))^,
             (PByte(Result) + (Cardinal(AnInsertSize) * FItemSize.ItemSize))^,
             (Cardinal(Cnt - AnInsertPos) * FItemSize.ItemSize)
        );
      end;
  end
  else begin
...
begin
  ...
  assert((MPtr^.FirstItemIdx >= 0) and (MPtr^.FirstItemIdx < FCapacityAccessor.ReadCapacity(MPtr)), 'TGenLazShiftList.InsertRows: (MPtr^.FirstItemIdx >= 0) and (MPtr^.FirstItemIdx < FCapacityAccessor.ReadCapacity(MPtr))');
end;
...
begin
  ...
  i := FCapacity.ShrinkCapacity(Cnt, FCapacityAccessor.ReadCapacity(MPtr));
  if i >= 0 then begin
    ...
    if AToIndex < AFromIndex then begin
      // free at end? (instead of moving entries down, move surroundings up
       if FCapacityAccessor.ReadCapacity(MPtr) - (MPtr^.FirstItemIdx + Cnt) >= AFromIndex - AToIndex then begin
        ...
        assert((MPtr^.FirstItemIdx >= 0) and (MPtr^.FirstItemIdx < FCapacityAccessor.ReadCapacity(MPtr)), 'TGenLazShiftList.MoveRows: (MPtr^.FirstItemIdx >= 0) and (MPtr^.FirstItemIdx < FCapacityAccessor.ReadCapacity(MPtr))');
...
begin
  ...
  Result := TPItemT(@NewMPtr^.Data + (NewIdx * FItemSize.ItemSize));
