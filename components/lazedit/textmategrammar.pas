...
begin
  ...
  Result := FRegExMatch.Exec(ATextStartOffset, AMatchMustStartBefore) and (FRegExMatch.MatchLen[0] > 0);
  if Result then
...
begin
  ...
  Result := FRegExMatchBegin.Exec(ATextStartOffset, AMatchMustStartBefore);
  if Result then begin
    ...
    if FRegExMatchBegin.MatchLen[0] <= 0 then begin
      ...
      if FRegExMatchEnd.Exec(ATextStartOffset) and
...
begin
  ...
  if (not r) and (BodyPos <= st^.SubTextLen) then begin
    ...
    if not (psfMatchEndInitialized in st^.Flags) then begin
      ...
      if FRegExMatchEnd.Exec(BodyPos) then begin
...
begin
  ...
  if (psfWhileInBeginLine in st^.Flags) then begin
    ...
    if not (psfWhileDone in st^.Flags) then begin
      ...
      if not FRegExMatchWhile.Exec(ACurTokenPos) then begin
        AStates.CallParentNextToken(AText, ACurTokenPos, ANextTokenPos, AnInitInfoOnly);
        exit;
      end;
